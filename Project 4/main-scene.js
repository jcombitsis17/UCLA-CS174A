                   ...
                   plastic_stars: new Material( texture_shader_2,    
                     ...
                     metal_earth: new Material( texture_shader_2,    
                      ...
                      black_hole: new Material( black_hole_shader ),
                             sun: new Material( sun_shader, { ambient: 1 } ),
                        planet_1: new Material( phong_shader, { ambient: 0, 
                                                                diffusivity: 0.9, 
                                                                specularity: 0.1, 
                                                                color: Color.of( 0.5,0.5,0.5,1 ) } ),
                        planet_2: new Material( phong_shader, { ambient: 0, 
                                                                diffusivity: 0,
                                                                specularity: 1,
                                                                color: Color.of( 0.25,0.25,0.25,1 ) } ),
  ...
  display( context, program_state )
    {                                                // display():  Called once per frame of animation.  For each shape that you want to
      ...
      **********************************/         
      ...
      let planet_1 = model_transform.times( Mat4.rotation( t, [ 0,1,0 ] ) )   // orbit
                                    .times( Mat4.translation( [ 5,0,0 ] ) )
                                    .times( Mat4.rotation( t, [ 0,1,0 ] ) );  // rotate
      ...
      let planet_2 = model_transform.times( Mat4.rotation( t/2, [ 0,1,0 ] ) )
      ...
      let moon_ec1 = planet_2.times( Mat4.rotation( t/2, [ 0,1,0 ] ) )
                             .times( Mat4.translation( [ 2,0,0 ] ) )
                             .times( Mat4.rotation( t, [ 0,1,0 ] ) );
      this.shapes.ball_6.draw( context, program_state, moon_ec1, this.materials.black_hole.override( modifier ) );
                                                // TODO (#4d3):  Draw planet 3 orbiting 3 units farther, revolving AND rotating slower.
      let earth = model_transform.times( Mat4.rotation( t/3, [ 0,1,0 ] ) )
                                 .times( Mat4.translation( [ 11,0,0 ] ) )
                                 .times( Mat4.rotation( t, [ 0,1,0 ] ) );
      this.shapes.ball_4.draw( context, program_state, earth, this.materials.earth.override( modifier ) );
                                                // TODO (#6b2):  Draw moon 2 orbiting 2 units away from planet 3, revolving AND rotating.
      let moon_2 = earth.times( Mat4.rotation( t, [ 0,1,0 ] ) )
                        .times( Mat4.translation( [ 2,0,0 ] ) )
                        .times( Mat4.rotation( t, [ 0,1,0 ] ) );
      this.shapes.ball_1.draw( context, program_state, moon_2, this.materials.moon_2.override( modifier ) );
                                                // TODO (#4d4):  Draw planet 4
      let planet_4 = model_transform.times( Mat4.rotation( t/4, [ 0,1,0 ] ) )
      ...
      let planet_5 = model_transform.times( Mat4.rotation( t/5, [ 0,1,0 ] ) )
      ...
      if ( this.lights_on )
      {
          for (let i = 0; i < this.star_matrices.length; i++)
              this.shapes.star.draw( context, program_state, this.star_matrices[i], this.materials.star );
      }
      ...
      let camera_adjust = Mat4.inverse( Mat4.rotation( t, [ 0,1,0 ] ) ).times( Mat4.translation( [ 0,0,3 ] ) );
      
      this.camera_teleporter.cameras.push( Mat4.inverse( model_transform.times( Mat4.translation( [ 0,0,8 ] ) ) ) );
      this.camera_teleporter.cameras.push( Mat4.inverse( planet_1.times( camera_adjust ) ) );
      this.camera_teleporter.cameras.push( Mat4.inverse( planet_2.times( camera_adjust ) ) );
      this.camera_teleporter.cameras.push( Mat4.inverse( moon_ec1.times( camera_adjust ) ) );
      this.camera_teleporter.cameras.push( Mat4.inverse( earth.times( camera_adjust ) ) );
      this.camera_teleporter.cameras.push( Mat4.inverse( moon_2.times( camera_adjust ) ) );
      this.camera_teleporter.cameras.push( Mat4.inverse( planet_4.times( Mat4.translation( [ 0,0,3 ] ) ) ) );
      this.camera_teleporter.cameras.push( Mat4.inverse( planet_5.times( Mat4.translation( [ 0,0,3 ] ) ) ) );
